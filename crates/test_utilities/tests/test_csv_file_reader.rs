use shared::*;
use readers_and_writers::CSVFileReader;


#[test]
/// Testing if a proper exception is raised when given path does not exists.
fn test_illegal_path() {
    assert!(CSVFileReader::new("non_existant_file", "".to_string()).is_err());
}
...
fn test_empty_file() -> Result<()> {
    assert!(
        CSVFileReader::new("tests/data/empty_file.tsv", "".to_string())?
            .get_header()
            .is_err()
    );
    assert!(
...
fn test_wrong_header() -> Result<()> {
    assert!(
        CSVFileReader::new("tests/data/macaque.tsv", "".to_string())?
            .get_column_number("kebab".to_owned())
            .is_err()
    );
    Ok(())

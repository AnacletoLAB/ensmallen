...
pub struct TypeFileReader<T: ToFromUsize + Sync> {
    pub reader: Option<CSVFileReader>,
    pub type_ids_column_number: Option<usize>,
    pub type_column_number: usize,
    pub types_number: Option<T>,
    pub numeric_type_ids: bool,
    pub minimum_type_id: Option<T>,
}
...
impl<T: ToFromUsize + Sync> TypeFileReader<T> {
    ...
    ///
    pub fn set_graph_name(mut self, graph_name: String) -> TypeFileReader<T> {
        self.reader
            .as_mut()

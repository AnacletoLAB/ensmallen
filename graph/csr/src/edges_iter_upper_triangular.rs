...
impl<'a> core::iter::Iterator for EdgesIterUpperTriangular<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut src_limit;
        loop {
            src_limit = self.outbounds[1 + self.start_src as usize];
        ...
        // convert UpperTriangular edge id to directed edge id
        let edge_id = self.father.outbounds_degrees[1 + self.start_src as usize]
            - (src_limit - self.start_edge_id);
...
impl<'a> core::iter::DoubleEndedIterator for EdgesIterUpperTriangular<'a> {
    fn next_back(&mut self) -> Option<Self::Item> {
        ...
        // if we finished the current src, skip singletons and go to the next
        loop {
            let src_start = self.outbounds[self.end_src as usize];
        ...
        // convert UpperTriangular edge id to directed edge id
        let src_end = self.outbounds[1 + self.end_src as usize];
        let edge_id =
            self.father.outbounds_degrees[1 + self.end_src as usize] - (src_end - self.end_edge_id);

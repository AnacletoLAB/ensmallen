...
/// The provided message was: 'The path from source to destination has distance 679230272984414300000000000000000000000 while the distance from destination to source has destination 679230272984414300000000000000000000000. The path from source to destination is [1, 0, 2], while the path from destination to source is [2, 0, 1]. The two paths should be symmetric and with the same distance.'
///
fn test_regression_50() -> Result<(), String> {
    let edges_reader = EdgeFileReader::new("tests/data/regression/50.edges")?

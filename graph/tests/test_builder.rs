...
fn test_builder() {
    for directed in &[true, false] {
        let graph = Graph::builder(vec![0, 1, 2, 3, 0, 0], vec![3, 1, 0, 0, 0, 1], *directed)
            .add_weights(vec![1.0, 1.0, 1.0, 1.0, 2.0, 3.0])
            .add_edge_types(
                vec![0, 0, 1, 1, 1, 0],
                [("PP".to_string(), 0), ("GG".to_string(), 1)]
                    .iter()
                    .cloned()
                    .collect(),
            ...
            .add_nodes(
                ...
                Some(vec![0, 0, 1, 1]),
                Some(
                    [("Protein".to_string(), 0), ("Gene".to_string(), 1)]
                        .iter()
                        .cloned()
                        .collect(),
                ),
...
#[test]
fn test_builder_no_nodes_mapping() {
    for directed in &[true, false] {
        let graph = Graph::builder(vec![0, 1, 2, 3, 0, 0], vec![3, 1, 0, 0, 0, 1], *directed)
            .add_weights(vec![1.0, 1.0, 1.0, 1.0, 2.0, 3.0])
            .add_edge_types(

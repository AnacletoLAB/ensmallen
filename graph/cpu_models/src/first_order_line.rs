...
impl FirstOrderLINE {
    /// Return new instance of first-order LINE model.
    ///
    /// # Arguments
    ...
    /// Computes in the provided slice of embedding the first-order LINE node embedding.
    ///
    ...
    pub fn fit_transform(
        ...
        let pb = if verbose {
            ...
            pb.set_style(ProgressStyle::default_bar().template(
                "First-order LINE {msg} {spinner:.green} [{elapsed_precise}] [{bar:40.cyan/blue}] ({pos}/{len}, ETA {eta})",
            ));
            pb
        } else {
            ProgressBar::hidden()

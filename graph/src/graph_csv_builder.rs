...
impl FromCsvBuilder {
    ...
    pub fn set_force_conversion_to_undirected(
        mut self
    ) -> FromCsvBuilder {
    ...
    pub fn build(
        ...
        for (line_index, _line) in lines.enumerate() {
            ...
            let edge_type = match &edge_type_name {
                None => None,
                Some(et_name) => {
                    Some(match edge_types_mapping.get(&et_name.clone()) {
                        Some(et) => *et,
                        None => {

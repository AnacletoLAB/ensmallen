...
impl Graph {
    ...
    /// Returns new graph without singleton node types.
    ///
    /// A node type is singleton when it appears in a single node.
    ///
    pub fn drop_singleton_node_types(&self) -> Result<Graph> {
        Ok(self
            .filter_from_ids(
                None,
                None,
                ...
                Some(
                    self.iter_singleton_node_type_ids()?
                        .map(|node_type_id| Some(node_type_id))
                        .collect::<Vec<_>>(),
                ),
                None,
                None,
    ...
    /// Returns new graph without singleton edge types.
    ///
    /// A edge type is singleton when it appears in a single edge.
    ///
    pub fn drop_singleton_edge_types(&self) -> Result<Graph> {
        Ok(self
            .filter_from_ids(
                None,
                None,
                ...
                Some(
                    self.iter_singleton_edge_type_ids()?
                        .map(|edge_type_id| Some(edge_type_id))
                        .collect::<Vec<_>>(),
                ),
                None,
                None,

...
pub(crate) struct AtomicF64HashMap<K: Eq + Hash + Copy + 'static> {
    value: UnsafeCell<HashMap<K, AtomicU64>>,
    insert_mutex: Mutex<()>,
    max_value: AtomicU64,
}
...
impl<K: Eq + Hash + Copy + 'static> AtomicF64HashMap<K> {
    ...
    pub fn into_iter(self) -> impl Iterator<Item = (K, f64)> {
        self.value.into_inner().into_iter().map(|(k, v)| {
    ...
    pub fn add(&self, key: &K, increment: f64) {
        let value = match unsafe { (*self.value.get()).get(key) } {
            Some(val) => val,
            None => {
                ...
                unsafe {
                    (*self.value.get())
                        .entry(*key)
                        .or_insert_with(|| AtomicU64::new(f64_to_u64(0.0)))
                }
            }
        };
        // update the value
        ...
        while u64_to_f64(stored) < u64_to_f64(new_value) {
            let inner = self
                .max_value
                .compare_and_swap(stored, new_value, Ordering::Relaxed);
            // if sucessfully updated exit
            if inner == stored {
                break;
            }

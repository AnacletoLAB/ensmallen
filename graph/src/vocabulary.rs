...
impl<IndexT: ToFromUsize> Vocabulary<IndexT> {
    ...
    pub fn from_reverse_map(reverse_map: Vec<String>) -> Result<Vocabulary<IndexT>> {
        let map = reverse_map.iter()
            .enumerate()
            .cloned()
        ...
        if map.len() != reverse_map.len() {
            // TODO extend the duplicate check returning
            // which value is duplicated
            return Err("Duplicated values found!");
        }
...
impl<IndexT: ToFromUsize> Vocabulary<IndexT> {
    ...
    pub unsafe fn unchecked_insert(&mut self, value: String) -> IndexT {
        match self {
            Vocabulary::String{ map, reverse_map} => {
                ...
                *map.entry(value).or_insert_with_key(|value| {
                    IndexT::from_usize(current_length)
                })
            }
    ...
    pub fn insert<S: AsRef<str>>(&mut self, value: S) -> Result<(IndexT, bool)> {
        ...
        if value.is_empty() {
            return Err("The given value is empty, we cannot insert an empty value into the vocabulary".to_string());
        }
        ...
        match self {
            Vocabulary::String{map, reverse_map} => {
                Ok(match map.entry(normalized_value) {
    ...
    pub fn translate(&self, id: IndexT) -> Result<String> {
        match self {
            Vocabulary::String{ reverse_map, ..} => {
                match reverse_map.get(IndexT::to_usize(id)) {
            ...
            Vocabulary::Numeric{range, ..} => {
                if range.contains(&IndexT::to_usize(id)) {
                    ...
                    Err("The requested id is over the range of the current numeric vocabulary.".to_string())
                }
            },
        }
    ...
    pub fn keys(&self) -> Vec<String> {
        match self {
            Vocabulary::String{ map, ..} => map.keys().cloned().collect(),
            Vocabulary::Numeric{range, ..} => {
                range.map(|i| format!("{}", i)).collect::<_>()
            },
        }
    }
        ...
        match self {
            ...
            Vocabulary::Numeric{range, ..} => {
                *self = Vocabulary::String{
                    map: range.map(|i| (format!("{}", i), IndexT::from_usize(i))).collect(),
        ...
        let result = match self {
            Vocabulary::Numeric{range, count} => {
                ...
                if i == j {
                    ...
                    let result = range.map(|i| if new_range.contains(&i) {
                        Some(i - min)
                    } else {
                        None

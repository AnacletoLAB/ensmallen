...
impl Graph {
    ...
    fn get_anchor_node_names_from_anchor_node_ids(&self, anchor_node_ids: &[Vec<NodeT>]) -> Vec<Vec<String>> {
        // If the node names are to be returned, we compute them.
        info!("Retrieving the sampled node names");
        anchor_node_ids.par_iter().map(|anchor_node_ids|{
            let mut anchor_node_names =
            vec!["".to_string(); anchor_node_ids.len()];
        anchor_node_ids
            .par_iter()
            .cloned()
    ...
    pub fn get_shortest_paths_node_embedding<T: TryFrom<u32> + Send + Sync>(
        ...
        verbose: Option<bool>,
    ) -> Result<(NodeT, impl Iterator<Item=impl IndexedParallelIterator<Item=T> + '_> + '_, Option<Vec<Vec<String>>>)> {
        ...
        Ok((
            ...
            anchor_node_ids.into_iter().progress_with(pb).map(move |anchor_node_ids| {

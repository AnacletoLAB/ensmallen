...
/// * `target_edge_list_path`: &str - The path from where to load the target edge list. This cannot be the same as the original edge list.
...
pub fn add_edge_id_to_edge_list(
    ...
    if target_edge_list_edge_ids_column.is_none()
        && target_edge_list_edge_ids_column_number.is_none()
    {
        return Err(concat!(
            "Either the target edge list edge ids column or the target edge list edge ids column number ",
            "must be provided in order to write the edge ids to the file."
        ).to_string());
    }

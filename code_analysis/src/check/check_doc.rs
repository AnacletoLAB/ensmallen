                ...
                if struct_name == "Graph" {
                    for method in &impls.methods {
                        ...
                        if method.has_arguments()
                            && find_section!(sections, DocSection::Arguments { .. }).is_none()
                        {
                            self.log(Error::MissingSection {
                                method_name: method_name.clone(),
                        ...
                        if INCLUDE_EXAMPLE
                            && find_section!(sections, DocSection::Example { .. }).is_none()
                        {
                            self.log(Error::MissingSection {
                                method_name: method_name.clone(),
                        ...
                        if method.is_unsafe()
                            && find_section!(sections, DocSection::Unsafe { .. }).is_none()
                        {
                            self.log(Error::MissingSection {
                                method_name: method_name.clone(),
                        ...
                        if method.returns_result()
                            && find_section!(sections, DocSection::Raises { .. }).is_none()
                        {
                            self.log(Error::MissingSection {
                                method_name: method_name.clone(),
                            ...
                            match sec {
                                ...
                                DocSection::Arguments {
                                    ...
                                    let method_args: Vec<_> = method
                                        ...
                                        .0
                                        .iter()
                                        .filter(|x| matches!(x.arg_type, Type::SelfType))
                                        .collect();

                                    // check that all the args are present and no extra arg is present
                                    let doc_names: BTreeSet<String> =
                                        parsed_args.iter().map(|x| x.name.clone()).collect();
                                    let arg_names: BTreeSet<String> =
                                        method_args.iter().map(|x| x.name.clone()).collect();

                                    let missing_args: Vec<String> =
                                        doc_names.difference(&arg_names).cloned().collect();
                                    let extra_args: Vec<String> =
                                        arg_names.difference(&doc_names).cloned().collect();
                                    let common_args: Vec<String> =
                                        arg_names.intersection(&doc_names).cloned().collect();
                                    ...
                                    for arg_name in common_args {
                                        let documented = parsed_args
                                            .iter()
                                            .find(|x| x.name == arg_name)
                                            .unwrap();
                                        let truth = method_args
                                            .iter()
                                            .find(|x| x.name == arg_name)
                                            .unwrap();

                                        if String::from(truth.arg_type.clone())
                                            != documented.arg_type
                                        {

...
pub const KEYWORDS: &[&str] = &["unweighted", "weighted", "unchecked", "naive"];
...
impl Checker {
    fn extract_regexes_from_impl(&self, imp: &Impl) -> Vec<Regex> {
        let result = IMPL_DOC_REGEX_REGEX
            .captures_iter(&imp.doc)
    ...
    fn check_method(&self, method: &Function, regexes: &[Regex]) {
        ...
        match (method.is_unsafe(), method.name.contains(&"unchecked")) {
            (true, false) => {
                self.log(Error::MissingUnchecked {
                    method_name: method.name.clone(),
                });
            }
            (false, true) => {
                self.log(Error::MissingUnsafe {
                    method_name: method.name.clone(),
                });
            }
            _ => {}
        ...
        // match them.
        let does_name_match = regexes.iter().any(|re| re.is_match(&method.name));

        // if regexes in the impl doc, the methods MUST follow the rule, can be disable with #[ignore_regex_name]
        if !regexes.is_empty()
            && !does_name_match
            && !method.attributes.iter().any(|x| x == "ignore_regex_name")
        {
            self.log(Error::MethodNameDoesNotMatchRegex {
                method_name: method.name.clone(),
                regexes: regexes
                    .iter()
        ...
        for keyword in KEYWORDS {
            ...
            if method.name.contains(keyword) && !METHOD_KEYWORD_REGEX.is_match(&method.name) {
                self.log(Error::WrongKeywordPositionInMethodName {
                    method_name: method.name.clone(),
        ...
        if method.name.contains("unknown")
            && !self
                .method_names
                .contains(&method.name.replace("unknown", "known"))
            && !method.attributes.iter().any(|x| x == "no_known")
        {
            self.log(Error::MissingDualMethod {
                method_name: method.name.clone(),
        ...
        if method.name.contains("known")
            && !self
                .method_names
                .contains(&method.name.replace("known", "unknown"))

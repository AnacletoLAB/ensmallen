...
fn get_headers(path: &str, sep: &str) -> Vec<String> {
    ...
    line.trim().split(sep).map(|c| String::from(c)).collect()
}

fn render_columns(columns: &Vec<String>, optional_columns: &Vec<Option<String>>) -> Vec<String> {
...
pub fn has_columns(
    ...
    sep: &str,
    columns: &Vec<String>,
    optional_columns: &Vec<Option<String>>,
) {

...
class StringGraphRepository(GraphRepository):
    ...
    def get_data(self) -> Dict:
        ...
        species_11_0 = pd.read_csv(
            "https://stringdb-downloads.org/download/species.v11.0.txt", sep="\t"
        )
        species_11_5 = pd.read_csv(
            "https://stringdb-downloads.org/download/species.v11.5.txt", sep="\t"
        )
        species_12_0 = pd.read_csv(
            "https://stringdb-downloads.org/download/species.v12.0.txt", sep="\t"
        )
        ...
        parameters_per_type_of_graph = {
            ...
            "enrichment.terms": {
                "sources_column": "#string_protein_id",
                "destinations_column": "term",
                "edge_list_edge_types_column": "category",
            },
            "homology": {
                "sources_column": "##protein1",
                "destinations_column": "protein2",
        ...
        # We are not any more retrieving the sequence data because they are way too big
        ...
        tree_url_pattern = (
            "https://stringdb-downloads.org/download/species.tree.v{version}.txt"
        )
        tree_metadata_url_pattern = (
            "https://stringdb-downloads.org/download/species.v{version}.txt"
        )
        ...
        complete_graph_info_node_list_url_pattern = (
            "https://stringdb-downloads.org/download/protein.info.v{version}.txt.gz"
        )
        ...
        for version in ("11.0", "11.5", "12.0"):
            ...
            mined_data[stored_graph_name][full_version_code] = {
                ...
                "paths": ["species.tree.tsv", "species.tsv"],
        ...
        for version in ("11.0", "11.5", "12.0"):
            complete_graph_info_node_list_url = (
                complete_graph_info_node_list_url_pattern.format(
                    version=version,
                )
            )

            complete_graph_enrichment_node_list_url = (
                complete_graph_enrichment_node_list_url_pattern.format(
                    version=version,
                )
            )
            mined_data[stored_graph_name][f"enrichment.terms.v{version}"] = {
                "urls": [
                    complete_graph_enrichment_node_list_url,
                ],
                "paths": [
                ...
                "arguments": {
                    ...
                    "edge_list_is_correct": True,
                    **parameters_per_type_of_graph["enrichment.terms"],
                },
            }
            ...
            for edge_list_version in (
                ...
                complete_graph_edgelist_url = (
                    complete_graph_edgelist_url_pattern.format(
        ...
        for version, species in (
            ...
            for _, row in species.iterrows():
                taxon_id = row[0]
                enrichment_graph_name = f"{row.STRING_name_compact} Enrichment"
                stored_enrichment_graph_name = self.build_stored_graph_name(
                    enrichment_graph_name
                )
                if stored_enrichment_graph_name not in mined_data:
                    mined_data[stored_enrichment_graph_name] = {}

                mined_data[stored_enrichment_graph_name][
                    f"enrichment.terms.v{version}"
                ] = {
                    "urls": [
                ...
                for type_of_graph in type_of_graphs:
                    edge_list_url = edge_list_url_pattern.format(
                        taxon_id=taxon_id, version=version, type_of_graph=type_of_graph
                    )
                    ...
                    mined_data[stored_graph_name][full_version_code] = {
                        ...
                        "paths": ["edges.tsv.gz", "nodes.tsv.gz"],
                        "arguments": {
                            "edge_path": "edges.tsv",
                            "node_path": "nodes.tsv",
                            "name": graph_name,
    ...
    def build_stored_graph_name(self, partial_graph_name: str) -> str:
        ...
        if " " in partial_graph_name:
            return "".join(
                [term.capitalize() for term in partial_graph_name.split(" ")]
            )
    ...
    def get_callbacks(self, graph_name: str, version: str) -> List[str]:
        ...
        callbacks.
        """
        params = self._data[self.build_stored_graph_name(graph_name)][version]
    ...
    def get_callbacks_arguments(self, graph_name: str, version: str) -> List[Dict]:
        ...
        """
        params = self._data[self.build_stored_graph_name(graph_name)][version]
        if "callback_arguments" in params:
            return [params["callback_arguments"]]
        return None
    ...
    def get_graph_references(self, graph_name: str, version: str) -> List[str]:
        ...
        return [open(f"{path}/models/string_citation.bib", "r", encoding="utf8").read()]
    ...
    def build_all(self):
        ...
        with open(file_path, "a", encoding="utf8") as f:
            with open(
                "graph_miner/repositories/models/parse_string.py", "r", encoding="utf8"
            ) as original:

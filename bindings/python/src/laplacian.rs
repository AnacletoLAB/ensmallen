...
impl Graph {
    ...
    #[args(py_kwargs = "**")]
    #[text_signature = "($self)"]
    /// Return tuple with edge node IDs and edge weights of the left normalized laplacian.
    pub fn get_left_normalized_laplacian_coo_matrix(
        ...
        let edge_weights = PyArray1::new(gil.python(), [edges_number], false);
        self.inner
            .iter_left_normalized_laplacian_coo_matrix()
            .enumerate()
    ...
    #[args(py_kwargs = "**")]
    #[text_signature = "($self)"]
    /// Return tuple with edge node IDs and edge weights of the right normalized laplacian.
    pub fn get_right_normalized_laplacian_coo_matrix(
        ...
        let edge_weights = PyArray1::new(gil.python(), [edges_number], false);
        self.inner
            .iter_right_normalized_laplacian_coo_matrix()
            .enumerate()
            .for_each(|(i, (src, dst, edge_weight))| unsafe {
                *edge_node_ids.uget_mut([i, 0]) = src;
                *edge_node_ids.uget_mut([i, 1]) = dst;

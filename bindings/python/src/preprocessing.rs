...
impl Graph {
    ...
    where
        F: Fn(u64, &mut [NodeT], &mut [NodeT], &mut [f32], Option<f64>) -> Result<()>,
    {
        let gil = pyo3::Python::acquire_gil();
...
impl Graph {
    ...
    fn get_edge_adamic_adar_mini_batch(
        ...
        self.populate_edge_score_mini_batch(
            random_state,
            batch_size,
            |random_state, srcs_slice, dsts_slice, scores_slice, negative_samples_rate| {
                self.inner.populate_edge_adamic_adar_mini_batch(
                    random_state,
                    srcs_slice,
                    dsts_slice,
                    scores_slice,
    ...
    #[text_signature = "($self, random_state, batch_size, negative_samples_rate)"]
    ...
    fn get_edge_jaccard_mini_batch(
        ...
        self.populate_edge_score_mini_batch(
            ...
            batch_size,
            |random_state, srcs_slice, dsts_slice, scores_slice, negative_samples_rate| {
                self.inner.populate_edge_jaccard_mini_batch(
                    random_state,
                    srcs_slice,
                    dsts_slice,
                    scores_slice,
    ...
    /// Returns source and destination nodes of an edge and their shared neighbours size.
    ///
    /// Parameters
    /// -------------
    /// random_state: int
    ...
    /// ValueError
    ///     If the given amount of negative samples is not a positive finite real value.
    fn get_neighbours_intersection_size_mini_batch(
        &self,
        random_state: u64,
        batch_size: usize,
        negative_samples_rate: Option<f64>,
        ...
        self.populate_edge_score_mini_batch(
            ...
            batch_size,
            |random_state, srcs_slice, dsts_slice, scores_slice, negative_samples_rate| {
                self.inner.populate_neighbours_intersection_size_mini_batch(
                    random_state,
                    srcs_slice,
                    dsts_slice,
                    scores_slice,
    ...
    #[text_signature = "($self, random_state, batch_size, bfs, negative_samples_rate)"]
    /// Returns source and destination nodes of an edge and their shared ancestor Jaccard.
    ///
    /// Parameters
    /// -------------
    /// random_state: int
    ...
    fn get_ancestors_jaccard_mini_batch(
        ...
        self.populate_edge_score_mini_batch(
            ...
            batch_size,
            |random_state, srcs_slice, dsts_slice, scores_slice, negative_samples_rate| {
                self.inner.populate_ancestors_jaccard_mini_batch(
                    random_state,
                    srcs_slice,
                    dsts_slice,
                    scores_slice,
    ...
    #[text_signature = "($self, random_state, batch_size, bfs, negative_samples_rate)"]
    /// Returns source and destination nodes of an edge and their shared ancestor size.
    ///
    /// Parameters
    /// -------------
    /// random_state: int
    ...
    /// ValueError
    ///     If the given amount of negative samples is not a positive finite real value.
    fn get_shared_ancestors_size_mini_batch(
        &self,
        random_state: u64,
        batch_size: usize,
        bfs: &ShortestPathsResultBFS,
        ...
        self.populate_edge_score_mini_batch(
            ...
            batch_size,
            |random_state, srcs_slice, dsts_slice, scores_slice, negative_samples_rate| {
                self.inner.populate_shared_ancestors_size_mini_batch(
                    random_state,
                    srcs_slice,
                    dsts_slice,
                    scores_slice,

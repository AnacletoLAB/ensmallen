...
pub fn create_memory_mapped_numpy_array(
    ...
    let description = format!(
        ...
        fortran_order=if fortran_order {
            "True"
        } else {
            "False"
        },
        shape=shape.iter().map(|x| format!("{}, ", x)).collect::<Vec<String>>().join(""),
    );
    ...
    // mmap the file
    let mut mmap = MemoryMapped::new(path, Some(aligned_len + data_size as usize)).expect("Could not mmap the file");
    mmap.get_slice_mut::<u8>(0, Some(aligned_len)).unwrap().clone_from_slice(&header);

    let data = mmap.get_slice_mut::<u8>(offset, None).
        expect("Could get the data slice type.").as_ptr();
...
pub fn load_memory_mapped_numpy_array(
    ...
    // check the magic
    let magic = mmap.get::<u64>(0).expect("Could not find the numpy object len.").to_be();
    ...
    let (header_start, header_length) = match major {
        1 => {
            let header_length = mmap.get::<u16>(8).expect("Could not find the numpy object len.").to_le();
            (10, header_length as usize)
        }
        2 | 3 => {
            let header_length = mmap.get::<u32>(8).expect("Could not find the numpy object len.").to_le();
    ...
    let header_bytes = mmap.get_slice::<u8>(header_start, Some(header_length)).expect("Could not get the header");
    ...
    let data = mmap.get_slice_mut::<u8>(header_start + header_length, None).
        expect("Could get the data slice type.").as_ptr();
